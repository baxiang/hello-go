...
import (
	...
	paymentV1 "kratos/api/payment/v1"
)
...
func main() {
	...
	orderUC := biz.NewOrderUseCase(repo.NewOrderRepo(dataData, logger), productUC, dataData.NATS, logger)
	paymentUC := biz.NewPaymentUseCase(repo.NewPaymentRepo(dataData, logger), orderUC, dataData.NATS, logger)

	httpServer := server.NewHTTPServer(cfg, logger)
	paymentService := service.NewPaymentService(paymentUC)
	paymentV1.RegisterPaymentServiceHTTPServer(httpServer, paymentService)
...
func loadConfig() *config.Config {
	...
	if path == "" {
		path = "configs/payment-service.yaml"
	}
